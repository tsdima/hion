import { addLinePoint, Point, Position2D } from './point'
import { SDK } from './sdk'
...
import { SelectManager } from './SelectManager'
  ...
  const UNDO_ADD_ELEMENT = 1;
	...
	export class UndoManager {
		...
		undo() {
			if (this.canUndo()) {
				for (const cmd of this.items[this.index].undo) {
					this._exec(cmd);
				}
				this.index--;
			}
		...
		redo() {
			if (this.canRedo()) {
				this.index++;
				for (const cmd of this.items[this.index].redo) {
					this._exec(cmd);
				}
			}
		}
		...
		addElement(element: Hion.SdkElement) {
			this._add(
				UNDO_ADD_ELEMENT,
		...
		delElement(selMan: SelectManager) {
			const ids: number[] = []
			const savedLinks: Array<UndoRecord> = [{type: UCMD_LOAD_FROM_TEXT, data: this.sdk.save(true)}]
			selMan.each(function(e){
				ids.push(e.eid);
				for (const i in e.points) {
					const point = e.points[i]
					if (!point.isFree() && !point.point.parent.isSelect()) {
						const p = point.isPrimary() ? point : point.point;
		...
		moveElementsEnd(selMan: SelectManager) {
			const dx = this.data.sx - selMan.items[0].x
			const dy = this.data.sy - selMan.items[0].y

			if (dx || dy) {
				const ids: number[] = []
				const savedLinks: Array<UndoRecord> = [{type: UCMD_MOVE_ELEMENTS, ids: ids, dx: dx, dy: dy}];
				selMan.each(function(e){
					ids.push(e.eid);
					for (const i in e.points) {
						const point = e.points[i];
