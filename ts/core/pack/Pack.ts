...
interface TranslateList {
  ...
  interface ElementTemplateList {
		[name: string]: ElementTemplate;
	}

  interface CoreModule {
		init: (element: Hion.SdkElement) => void;
	}
  ...
  interface Make {
	...
	export class Pack {
      ...
      console.log('Load pack:', this.name)
		...
		initElement(element: Hion.SdkElement) {
			if (this.parent) {
		...
		getPropertyEditor(propType: number): CustomEditor {
