...
import { FLAG_USE_EDIT, SDK } from '../../core/sdk'
import { Panel } from '../controls/Panel'
import { Builder } from '../Builder'
import { GetPos, printError, toStep } from '../../tools/tools'
import { palette, popupElement } from '../../main'
import { SdkEditor } from '../../core/sdk-editor/sdkeditor'
...
export class FormEditor {
  ...
  private handleToolsMouseDown = (event: MouseEvent): boolean => {
    const p1 = GetPos(this.toolsLayer.element)
    ...
    if (frm_ms.state === FormOperationState.FRM_NONE) {
      const arr = this.getControlByPos(rx-2, ry-2)
  ...
  private handleToolsMouseMove = (event: MouseEvent): boolean => {
    const dx = toStep(event.clientX - frm_ms.x)
    const dy = toStep(event.clientY - frm_ms.y)
    switch (frm_ms.state) {
      ...
      case FormOperationState.FRM_SELECT:
        const p1 = GetPos(this.toolsLayer.element)
        const s = this.selector.element
        const _dx = event.clientX - frm_ms.startX
        const _dy = event.clientY - frm_ms.startY
        if(_dx < 0) {
        ...
        s.style.width = Math.abs(_dx) + "px"
        s.style.height = Math.abs(_dy) + "px"
        break;
      ...
      default:
        if (dx||dy) {
          for (const obj of frm_ms.dctrl) {
            const p = obj.el.props;
  ...
  private createParentGrips() {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      d.on("onmousedown", (event: MouseEvent) => {
  ...
  update() {
    ...
    this.ctrl = [];
    const selected = [];
    for (let element of this.sdk.imgs) {
      if (element !== this.parentElement) {
        const e = element.run(FLAG_USE_EDIT);
        if (e) {
          this.elements.add(e);
          if (element.sdk) {
            element = element.sdk.imgs[1];
          }
          const control: FEItem = {el: element, ctl: e};
  ...
  private isSelect(ctl: FEItem) {
    if (!frm_ms.dctrl) {
      return false;
    }

    for (const c of frm_ms.dctrl) {
      if (c === ctl) {
        return true;
      }
    }
  ...
  removeElement(element: Hion.SdkElement) {
    if (frm_ms.dctrl) {
  ...
  private getControlByPos(x: number, y: number): FEItem[] {
    for (let i = this.ctrl.length-1; i >= 0; i--) {
      const p = this.ctrl[i].ctl;
  ...
  private getControlsInRect(x1: number, y1: number, x2: number, y2: number) {
    const result: FEItem[] = []
  ...
  private getRect(objs: FEItem[]) {
    let x1 = 32768, y1 = 32768, x2 = 0, y2 = 0;
    for (const i in objs) {
      const o = objs[i].ctl;
      if(o.left < x1) {
        x1 = o.left;
  ...
  private moveSelection(dx: number, dy: number) {
    for (const obj of frm_ms.dctrl) {
      const p = obj.el.props;
      let chageHeight = false
      let chageWidth = false
  ...
  private updateLines() {
    ...
    const rect = this.getRect(frm_ms.dctrl);

    const bindFlags = this.bindFlags;
    function getEdges(x1: number, y1: number, ctl: FEItem) {
      const props = ctl.el.props;
      const radius = 6;

      let ex1 = props.Left.value
      let ex2 = props.Left.value + ctl.ctl.width
      let ey1 = props.Top.value
      let ey2 = props.Top.value + ctl.ctl.height
    ...
    let xAzis = false
    let yAzis = false
    for (const ctl of this.ctrl) {
      if (!ctl.el.isSelect()) {
        for (const p of points) {
          const edges = getEdges(p.x, p.y, ctl);
          if (edges.x > -1 && !xAzis) {
            xAzis = true;
            const node = new Builder().div("frm-ctl axis-x");
            node.move(edges.x, 0);
          ...
          if (edges.y > -1 && !yAzis) {
            yAzis = true;
            const node = new Builder().div("frm-ctl axis-y");
            node.move(0, edges.y);
            this.mainContainer.element.insertBefore(node.element, this.selector.element);
            this.nodes.push(node);
